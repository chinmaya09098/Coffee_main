...
| Priority | Location |
|---|---|
| 1 | Path stored in `CODEX_KEYS_CONFIG` environment variable |
| 2 | Same directory as the executable |
| 3 | `..\..\keys.config` relative to the executable |
| 4 | `..\..\..\keys.config` relative to the executable |

### Environment Variables

All keys can be supplied as environment variables instead of (or alongside) `keys.config`.
...
| `CODEX_KEYS_CONFIG` | Custom path to a `keys.config` file |

**Set for the current PowerShell session only:**

```powershell
$env:AZURE_OPENAI_ENDPOINT = "https://<resource>.openai.azure.com/openai/responses?api-version=2025-04-01-preview"
$env:AZURE_OPENAI_KEY      = "your-key-here"
$env:AZURE_DEPLOYMENT      = "your-deployment-name"
$env:GITHUB_TOKEN          = "github_pat_..."
$env:CODEX_KEYS_CONFIG     = "C:\custom\path\keys.config"
```

**Set permanently for your Windows user account:**

```powershell
[System.Environment]::SetEnvironmentVariable("AZURE_OPENAI_ENDPOINT", "https://<resource>.openai.azure.com/...", "User")
[System.Environment]::SetEnvironmentVariable("AZURE_OPENAI_KEY",      "your-key-here",   "User")
[System.Environment]::SetEnvironmentVariable("AZURE_DEPLOYMENT",      "deployment-name", "User")
[System.Environment]::SetEnvironmentVariable("GITHUB_TOKEN",          "github_pat_...",  "User")
```

**Set using Command Prompt (cmd.exe) — current session:**
...
**Set using Command Prompt — persistent (user scope):**

```cmd
setx AZURE_OPENAI_KEY "your-key-here"
